    ...
    const fetchProgress = async () => {
      try {
        const [studentResponse, submissionsResponse] = await Promise.all([
          fetch(`https://eduforall.vercel.app/api/students/${student_id}`, {
            headers: {
          ...
          fetch(`https://eduforall.vercel.app/api/submissions/${student_id}`, {
            headers: {
              'Content-Type': 'application/json',
              'Role': 'tutor'
              // 'Authorization': `Bearer ${authTokens.accessToken}`
        ...
        const totalLessons =  await fetch (`https://eduforall.vercel.app/api/v1/lessons/`, {
          headers: {
            'Content-Type': 'application/json',
            'Role': 'tutor'
            // 'Authorization': `Bearer ${authTokens.accessToken}`
