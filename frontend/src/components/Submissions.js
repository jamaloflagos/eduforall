...
const Submissions = ({assignment}) => {
    ...
    useEffect(() => {
        const fetchSubmissions = async () => {
          ...
          try {
            const res = await fetch(`https://eduforall-backend.vercel.app//api/v1/submissions/${assignment.id}/assignment`, {
                headers: {
    ...
    const getSubmissionById = async (submission_id, student_id) => {
        try {
            const res = await fetch(`https://eduforall-backend.vercel.app//api/v1/submissions/${submission_id}/student/${student_id}`, {
                headers: {
                  'Content-Type': 'application/json',
                  'Role': 'tutor'
                }
